...
export function registerSessionTools(server: McpServer, db: Firestore) {
  server.tool(
    ...
    async ({ project_id, session_type, metadata }) => {
      ...
      activeSessions.docs.forEach((doc: QueryDocumentSnapshot) => {
...
async function buildHandoffContext(db: Firestore, project_id: string) {
  ...
  const activeStories = activeTasks.filter((t: { task_type: string | null }) => t.task_type === "story");
  const activeDesigns = activeTasks.filter((t: { task_type: string | null }) => t.task_type === "design");
  ...
  const taskIds = new Set(tasksSnap.docs.map((d: QueryDocumentSnapshot) => d.id));
