...
| **Tasks** | `board_get_tasks`, `board_get_task`, `board_create_task`, `board_update_task` (supports project moves + parent-close orphan guard since 2.0.0), `board_bulk_update_tasks` (1-100 at once, same orphan guard), `board_delete_task` (safety-guarded) |
| **Sessions** | `board_create_session` (returns last session's handoff), `board_end_session`, `board_get_handoff` |
| **Activity** | `board_log_activity`, `board_get_activity` (cursor-paginated, filterable), `board_delete_activity`, `board_bulk_delete_activity` (filter-required, bounded by `max_delete`) |
| **Gate events** (2.2.0; 2.2.1 hardens ingest: bad rows quarantined whole, handled-prefix watermark, poison brake, `.ingest-state.json`) | `board_query_gate_events`, `board_ingest_gate_events` |
| **Shared memory** | `board_save_shared_memory`, `board_search_shared_memories`, `board_delete_shared_memory` — a team-wide store, visible to every other agent on their next search |

Twenty-one tools, one job: give the agent a place to put state that isn't the conversation. `tools/list` against your own build is the authority if this table ever drifts.
...
**`gate_events` (added 2026-09-16, 7 indexes):** `dev+ts↓`, `hook+ts↓`, `event+ts↓`, `hook+event+ts↓`, `dev+hook+ts↓`, `path+ts↓`, `paths(array-contains)+ts↓` — every filter combination `board_query_gate_events` issues. Missing one surfaces as `FAILED_PRECONDITION`; the tool returns `error: missing_composite_index` with the console link instead of throwing.
